    ...
    For oSteel in oXML.selectNodes("/" ns("PXML_Document","Order","Product","Slab","Steel")) {
        ...
        dSteelData :=  {n10: 0, n8: 0, n6: 0, n5: 0, n4: 0}

        If (oSteel.getAttribute("Type") == "none") {
            fAbort(hasExtra, A_ThisFunc, "Два комплекта усилении̌?", { "pFile": pFile }) 
            sSteelType := "Extra", hasExtra := true
        } else fAbort((oSteel.getAttribute("Type") != "mesh"), A_ThisFunc, "This steel node's attribute isn't ""mesh"" or ""none"".", { pFile: pFile })

        For oBar in oSteel.selectNodes(ns("Bar")) {
            If (sSteelType == "Extra")
                fAbort((oBar.selectNodes(ns("Segment")).length > 1), A_ThisFunc,"An extra reinforcement bar has more than one segment.")
            else if ( (sSteelType != "Bent") and (oBar.selectNodes(ns("Segment", "BendY")).length > 1) ) {
                fAbort(hasBent, A_ThisFunc, "Две гнутые сетки?", { "pFile": pFile })
                sSteelType := "Bent", hasBent := true
            }
            ...
            dSteelData[("n" nBarDiam)] += nBarLeng * nBarQuan
            dOutput.dTotal[("n" nBarDiam)] += nBarLeng * nBarQuan
        }
        
        If ((sSteelType != "Extra") and (sSteelType != "Bent")) {
            fAbort(hasFlat, A_ThisFunc, "Две плоские сетки?", { "pFile": pFile })
            sSteelType := "Flat", hasFlat := true
        }

        dOutput[("d" sSteelType)] := dSteelData.Clone()
    }
